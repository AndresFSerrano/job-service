...
class JobServiceClient:
    ...
    def register_client(self, client: JobClientConfig) -> dict[str, Any]:
        return self._request("POST", "/api/v1/job-clients", json=client.__dict__)

    def register_definition(self, definition: JobDefinitionConfig) -> RegisteredJobDefinition:
        payload = definition.__dict__
    ...
    def register_service(
        ...
        payload = client.__dict__.copy()
        payload["job_definitions"] = [definition.__dict__ for definition in definitions]
    ...
    def create_execution(
        ...
        *,
        ...
        definitions = self.list_definitions(job_key=job_key)
        if not definitions:
            raise JobServiceError(f"Definition not registered for {job_key}")
        definition = self._get_definition(definitions[0]["client_key"], job_key)
        if definition.config.payload_schema:
            self._validate_schema(definition.config.payload_schema, job_input or {}, "job_input")
        ...
        resp = self._request("POST", "/api/v1/job-executions", json=payload_body)
        return resp
    ...
    def list_executions(
        ...
        raw = self._request("GET", "/api/v1/job-executions")

...
@pytest.mark.asyncio
async def test_proxy_returns_503_on_pool_timeout():
    app = _build_app()
    fake_client = AsyncMock()
    fake_client.is_closed = False
    fake_client.get = AsyncMock(side_effect=httpx.PoolTimeout("busy"))
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == "El proxy de jobs esta saturado temporalmente."
